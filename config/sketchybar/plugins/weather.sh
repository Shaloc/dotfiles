...
QWEATHER_WARNING_API="https://devapi.qweather.com/v7/warning?location=${LOCATION}&key=${QWEATHER_API_KEY}"
...
if [[ $wind_scale -gt 3 ]]; then
    WEATHER_LABEL="${WEATHER_LABEL} ${wind_dir}${wind_scale}级 ${wind_speed}Km/h"
fi
if [[ $aqi -gt 50 ]]; then
